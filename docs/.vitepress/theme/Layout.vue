...
async function search(){
  ...
  catch{searchError.value='搜索索引加载失败，请重试'}
}
...
<template>
  ...
  <main id="main" :class="['main',{'editor-main':fm.layout==='editor'}]">
    ...
    <template v-else-if="fm.layout==='articles'||fm.layout==='topics'">
      ...
      <div class="tag-filters" aria-label="按专题筛选"><button :class="{selected:!selectedTag}" @click="selectedTag=''">全部</button><button v-for="tag in tags" :key="tag" :class="{selected:selectedTag===tag}" @click="selectedTag=tag">{{tag}} <span>{{posts.filter(p=>p.tags.includes(tag)).length}}</span></button></div>
    ...
    <template v-else-if="fm.layout==='article'">
      <div class="article-layout">
        <article class="article"><a class="back-link" :href="withBase('/articles.html')">所有文章</a><header class="article-heading"><h1>{{fm.title}}</h1><div class="post-meta"><time>{{String(fm.date).slice(0,10)}}</time><span v-for="tag in fm.tags" :key="tag" class="tag">{{tag}}</span><button class="print-button" @click="print"><Printer :size="15"/>导出 PDF</button></div></header>
          ...
          <section v-if="fm.pdfs?.length" class="original-pdfs"><h2>原 PDF</h2><div v-for="pdf in fm.pdfs" :key="pdf.url"><a :href="pdf.url" target="_blank" rel="noopener noreferrer">打开 {{pdf.name}}</a><a class="button small" :href="pdf.url" download>下载原文件</a><iframe :src="pdf.url" :title="pdf.name+' 原 PDF 阅读器'" loading="lazy"></iframe><small>若设备不支持内嵌阅读，请直接打开或下载。</small></div></section>
        </article>
    ...
    <template v-else-if="fm.layout==='about'"><div class="about-page"><span class="eyebrow">ABOUT</span><h1>关于</h1><p>这里用来存放学习笔记与阅读记录。</p><p class="muted">作者介绍待补充。站内标有“示例”的文章是功能演示，可随时替换。</p><div class="about-rule"></div><p class="small muted">首页摄影：<a href="https://unsplash.com/photos/misty-lake-with-mountains-and-trees-at-dawn-uyxJl4SBE3s" target="_blank" rel="noopener noreferrer">Anthony Gomez / Unsplash</a></p></div></template>
